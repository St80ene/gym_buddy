...
# Training data based on provided FAQ, greetings, nutrition, and routines
training_data = [
    # Greetings
...
exit_conditions = ("exit", "quit", "bye", "close", "goodbye")

print("Welcome to Gym Buddy! Type 'exit' to quit.")
while True:
    try:
        user_input = input("You: ")
        if user_input in exit_conditions:
            print("Thank you for using Gym Buddy! Goodbye!")
            break
        
        response = chatbot.get_response(user_input)
        
        # Check if the response is a default response
        if response.confidence < 0.5:  # Adjust confidence threshold as needed
            print("Gym Buddy: I'm not quite sure how to answer that. Here are some questions you might ask:")
            suggestions = [
